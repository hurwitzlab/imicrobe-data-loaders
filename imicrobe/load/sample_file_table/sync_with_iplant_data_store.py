...
with session_manager_from_db_uri(db_uri=os.environ['IMICROBE_DB_URI']) as imicrobe_db_session:
    for sample_file_group in util.grouper(imicrobe_db_session.query(models.Sample_file).limit(10), n=100):

        with irods.irods_session_manager() as irods_session:
            for sample_file in sample_file_group:
                sample_file_i += 1
                print('{}: "{}"'.format(sample_file_i, sample_file.sample_file_type))
